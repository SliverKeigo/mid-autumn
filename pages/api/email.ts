  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        return res.status(400).json({ success: false, message: '無效的郵箱地址' })
      }

      const emails: string[] = await kv.get('emails') || []

      if (emails.includes(email)) {
        return res.status(409).json({ success: false, message: '該郵箱已存在' })
      }

      emails.push(email)
      await kv.set('emails', emails)
      
      res.status(200).json({ success: true, message: '郵箱添加成功' })
    } catch (error) {
    ...
    res.status(405).json({ success: false, message: '不支持的請求方法' })
  }
}